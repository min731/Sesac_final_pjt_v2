...
def recommed_by_lname_gname(ans,user_input):
    ...
    if result == None:
        ...
        name = ['T_BOOK_CD', 'LIBRARY_NM', 'LIB_BOOK_CD', 'ISBN', 'TITLE', 'AUTHOR', 'GENRE', 'RENT_YN', 'B_IMG', 'B_INTRO', 'RENT_SUM']
...
def check_is_in_library(ans,node):
    ...
    if req_bname != '':
        if req_bname not in all_bname_list:
            ans.set_response(f'{req_bname}는 관내에 없는 도서입니다.\n다른 도서관을 이용해주세요.')
    ...
    if req_wname != '':
        if req_wname not in all_wname_list:
            ans.set_response(f'{req_wname}의 도서는 현재 관내에 없습니다. \n다른 도서관을 이용해주세요.')
...
def check_can_borrow(ans,node):
    ...
    if in_lib_bname != '' or in_lib_wname != '' or in_lib_lname != '':
        ...
        if in_lib_lname != '' and in_lib_bname != '' and in_lib_wname != '':
            ans.set_response(f'{ans.get_req_lname()}, {ans.get_req_bname()}, {ans.get_req_wname()}(으)로 검색된 도서입니다.')
        
        elif in_lib_bname != '' and in_lib_wname != '':
            ans.set_response(f'{ans.get_req_bname()}, {ans.get_req_wname()}(으)로 검색된 도서입니다.')
        
        elif in_lib_lname != '' and in_lib_wname != '':
            ans.set_response(f'{ans.get_req_lname()}, {ans.get_req_wname()}(으)로 검색된 도서입니다.')
        
        elif in_lib_lname != '' and in_lib_bname != '':
            ans.set_response(f'{ans.get_req_lname()}, {ans.get_req_bname()}(으)로 검색된 도서입니다.')
