...
class Node:
    ...
    def task(self,graph,node,user_input,ans,btn_intent):
        ...
        if node.get_key() == 1:
            ...
            ans, input_label, user_input = task.intent_classify(ans, intent_classify_model,user_input)
            ...
            if btn_intent == -1:
                ...
                # 문의
                    ...
                    print("next_node = node4")
                    next_node = graph[1][2] #node4
                    rmv_idx = 2
                    
                # 예외 (negative)
                elif input_label == 3:
                    print("next_node = node5")
                    print("의도분류 - negative 구현중...")
        ...
        # 의도 분류(조회) : 도서명 or 작가명 요청
            ...
            ans  = task.check_bname_wname(ans,node.get_data()['user_input'])
        ...
        # 의도 분류(추천) : 모델링 중
            ...
            ans = task.recommed_by_gname(ans,user_input)
        ...
        # 의도 분류(문의) : 문장 유사도 계산 후 답변 출력
            ...
            ans = task.check_inquiry_ans(ans,user_input, sbert_model, emd_csv ,emd_pt)
            next_node = None
        ...
        # DB에 책 있는지 확인
            ...
            ans, can_search  = task.check_is_in_library(ans,node)
        ...
        # DB 검색 성공, 대출 가능 or 불가능 판단
            ...
            ans = task.check_can_borrow(ans,node)
            ...
            next_node = None
            # next_node.set_data('search_bname',ans.get_search_bname())
            # next_node.set_data('search_wname',ans.get_search_wname())
    
        # DB 검색 실패 , node1으로
        elif node.get_key() == 8:

            print("<System> DB 검색 실패")
            ans.set_response("챗봇 : 현재 관내(DB)에 없는 도서명 혹은 작가명입니다.다른 도서관을 이용해 주세요.")
            # next_node = None
        ...
        # elif node.get_key() == 9:
            ...
            # want_borrow_bname = node.get_data()['search_bname']
            # want_borrow_wname = node.get_data()['search_wname']

            # ans = task.check_want_borrow(ans,node)
        ...
        #     ans = task.check_want_alarm(ans, want_alarm_bname,want_alarm_wname)
